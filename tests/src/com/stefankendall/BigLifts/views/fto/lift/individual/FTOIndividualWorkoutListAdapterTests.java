...
public class FTOIndividualWorkoutListAdapterTests extends BLTestCase {
    public void testBuildsHeadersAndSetsForWarmupAndWorksets() {
        ...
        assertEquals(adapter.getCount(), TOOLBAR + WARMUP_HEADER + WARMUP_SETS + WORK_SETS_HEADER + WORK_SETS);
    }
    ...
    public void testGetsSetRowFromPosition() {
        ...
        FTOIndividualWorkoutListAdapter adapter = new FTOIndividualWorkoutListAdapter(null, jftoWorkout);
        assertEquals(adapter.setNumberForPosition(0), -1);
        assertEquals(adapter.setNumberForPosition(1), -1);
        assertEquals(adapter.setNumberForPosition(2), 0);
        assertEquals(adapter.setNumberForPosition(3), 1);
        assertEquals(adapter.setNumberForPosition(4), 2);
        assertEquals(adapter.setNumberForPosition(5), -1);
        assertEquals(adapter.setNumberForPosition(6), 3);
        assertEquals(adapter.setNumberForPosition(7), 4);
        assertEquals(adapter.setNumberForPosition(8), 5);
        assertEquals(adapter.setNumberForPosition(9), -1);
        assertEquals(adapter.setNumberForPosition(10), 6);
    }
}
