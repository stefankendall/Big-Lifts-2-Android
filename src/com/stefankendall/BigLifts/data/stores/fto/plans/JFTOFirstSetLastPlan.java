...
public class JFTOFirstSetLastPlan implements JFTOPlan {
    public Map<Integer, List<JSetData>> generate(JLift lift) {
        ...
        firstSetLastSets.put(2, Lists.newArrayList(
                JSetData.builder().withReps(5).withPercentage(new BigDecimal("70")).withLift(lift).withAmrap(true)
        ));

        firstSetLastSets.put(3, Lists.newArrayList(
                JSetData.builder().withReps(5).withPercentage(new BigDecimal("75")).withLift(lift).withAmrap(true)
        ));
